...
export class PanelWidgetClipPathComponent implements OnInit {
	...
	ngOnInit() {
		const _clip_path = this.widget.getObjKeyValue("panelClipPathModel");
		if (_clip_path) {
			if (_clip_path instanceof ClipInsetModel) {
				this.clipPathResizeMaskService.createInsetClip(<ClipInsetModel>_clip_path);
			} else if (_clip_path instanceof ClipEllipseModel) {
				this.clipPathResizeMaskService.createEllipseClip(<ClipEllipseModel>_clip_path);
			} else if (_clip_path instanceof ClipCircleModel) {
				this.clipPathResizeMaskService.createCircleClip(<ClipCircleModel>_clip_path);
			} else if (_clip_path instanceof ClipPolygonModel) {
				this.clipPathResizeMaskService.createPolygonClip("", <ClipPolygonModel>_clip_path);
			}
		}
	}
	...
	public createInsetClip(): void {
		...
		if (this.widget) {
			const _inset = this.clipPathResizeMaskService.clipInsetModel$.value;
			this.widget.addObjKeyValue({ panelClipPathModel: _inset });
			this.widget.addStyleToUltimatelyStyle(_inset.styleContent);
		}
	}
	...
	public createCircleClip(): void {
		...
		if (this.widget) {
			const _circle = this.clipPathResizeMaskService.clipCircleModel$.value;
			_circle.handleCalcCircleSidePoint(this.widget.profileModel);
			this.widget.addObjKeyValue({ panelClipPathModel: _circle });
			this.widget.addStyleToUltimatelyStyle(_circle.styleContent);
		}
	}
	...
	public createEllpiseClip(): void {
		...
		if (this.widget) {
			const _ellipse = this.clipPathResizeMaskService.clipEllipseModel$.value;
			this.widget.addObjKeyValue({ panelClipPathModel: _ellipse });
			this.widget.addStyleToUltimatelyStyle(_ellipse.styleContent);
		}
	}
	...
	public createPolygon(type: TPolygonType): void {
		...
		if (this.widget) {
			const _polygon = this.clipPathResizeMaskService.clipPolygonModel$.value;
			this.widget.addObjKeyValue({ panelClipPathModel: _polygon });
			this.widget.addStyleToUltimatelyStyle(_polygon.styleContent);
		}
	}
	...
	public handleEllipseClipDragPoint(drag: DraggablePort, point: ClipPointModel): void {
		if (point.type == "ellipse-r") {
			...
			const _radius_x = Math.abs(this.clipEllipsePath.ellR.left - this.clipEllipsePath.ellC.left);
			const _radius_y = Math.abs(this.clipEllipsePath.ellB.top - this.clipEllipsePath.ellC.top);
			point.left = this.calcPer(drag, point.left);
			point.top = this.calcPer(drag, point.top, "height");
			...
			if (point.left >= 50) {
				this.clipEllipsePath.ellR.left = point.left - _radius_x;
			} else if (point.left < 50) {
				this.clipEllipsePath.ellR.left = point.left + _radius_x;
			}
			if (point.top >= 50) {
				this.clipEllipsePath.ellB.top = point.top - _radius_y;
	...
	public handleCircleClipDragPoint(drag: DraggablePort, point: ClipPointModel): void {
		if (point.type == "circle-c") {
			...
			const _k = this.clipCirclePath.calcSlopeK(this.widget.profileModel);
			const _b = this.clipCirclePath.calcSlopeB(this.widget.profileModel);
			point.left += drag.left;
			point.top = this.widget.profileModel.height - (_k * point.left + _b);
	...
	public handleInsetClipDragPoint(drag: DraggablePort, point: ClipPointModel): void {
		...
		if (_math_obj[point.type]) {
			_math_obj[point.type]();
		}
